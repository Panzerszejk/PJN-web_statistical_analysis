...
def index(request):
    ...
    if request.GET.get('submit'):
        if request.GET.get('search') == "":
            ...
            if words.issubset(term_weights):
                ...
                if len(words) == 0:
                    ...
                    if request.GET.get('lemmatization') == 'on':
                        term_weightsc = term_weights_lemmas
                        words_c = words.copy()
                        words.clear()
                        ...
                        term_weightsc = term_weights
                    for word in words:
                        dict_list.append(term_weightsc[word])
                    temp = dict_list.pop()
                    ...
                    for file in temp:
                        result[file] = 0
                        for word in words:
                            result[file] += term_weightsc[word][file]
                    result_result = []
